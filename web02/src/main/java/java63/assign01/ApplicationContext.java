...
public class ApplicationContext {
  ...
  private void prepareObjects(String packageName) throws Exception {
	  //packageName 에  "java63.assign01"들어간다.
    ...
    for (Class<?> clazz : clazzList) {
      ...
      if (compAnno.value().equals("")) {
        ...
        System.out.println("objPooll"+objPool);
      } else {
        objPool.put(compAnno.value(), instance);
        System.out.println("objPoollllllllll"+objPool);
      }
    }
  }
  ...
  public void injectDependency() throws Exception {
    ...
    for (Object obj : objPool.values()) {
      ...
      for (Method m : methods) {
        ...
        if (m.getName().startsWith("set")) { //ex. setProductDao
          log.debug("   " + m.getName());//♥setProductDao/setSqlSessionFactory
        ...
        //dependency=  java63.assign01.ProductDao@1084709
      
          log.debug("      " + m.getParameterTypes()[0].getName());//☆java63.assign01.ProductDao
