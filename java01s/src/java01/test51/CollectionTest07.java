/*  Iterator 설계 비법
 * - 보관소에서 데이터를 꺼낼 때 사용하는 설계방식
 *      -> 데이터를 꺼내주는 담장자!
 *      
 * - 목적: 
 * 		배열에서 꺼내든, 링크드리스트에서 꺼내든, 스택에서 꺼내든,
 * 		저장소의 구조에 상관없이 
 * 		동일한 인터페이스(동일한 이름의 메서드)를 사용하여
 * 		데이터를 꺼내게 한다.
 *  */
...
class MyLinkedList4 {
	/* member inner class
	 * 멤버 inner class는 바깥 클래스의 인스턴스에 접근할 수 있다.
	 * 메서드처럼 생각하면 된다.*/
	class Iterator{ //목록ㅇㅔ서 데이터를 꺼내줌
		Bucket cursor = start;
...
public class CollectionTest07 {
  public static void printArray(MyLinkedList4 list) {
   ...
   while(iterator.hasNext()){   //꺼낼꺼잇냠?
	   System.out.println(iterator.next()); //꺼내줭 >_<
   }
   //innerclass 쓸때는 바깥클래스쩜. inner
  }
  
  public static void testGet(MyLinkedList4 arr){
	  Object value = null;
	  System.out.println("get(i)사용하긔--------------------- ");
	    ...
	    System.out.println("Iterator사용하기------------------------");
