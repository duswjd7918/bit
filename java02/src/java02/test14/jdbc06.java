/* getXXX(컬럼인덱스) 사용
 컬럼인덱스는 1부터 시작해  getInt(1)
  -> 문제점!!!!   인덱스는 코드를 해석하기 어렵게 만든다. 1234... 권장 ㄴ
   
   select * from 테이블명.
 컬럼선택절에 와일드카드(*)사용하면 속도가 느리당.. 
...
public class jdbc06 {
  ...
  public static void main(String[] args) {
    ...
    try {
     ...
     //4.select문 실행하기 
     // =return=> 서버에서 결과를 하나씩 가져오는 역할자를 리턴한다. 
      // 즉, java.sql.ResultSet 구현체를 리턴한다.
     rs = stmt.executeQuery("SELECT * from PRODUCTS");
     System.out.println("서버에 질의 완료. ResultSet 준비.");
     
     //5.결과 가져오기
     //->현재 결과를 가져온 상태가 아님.
     while(rs.next()){
       //결과를 가져왔다면, 데이터는 ResultSet객체에 들어잇다.
